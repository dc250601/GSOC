...
def hybrid_trainer(instance_name = "default"):
    model = hybrid.Hybrid()
    ...
    dataloader_train = torch.utils.data.DataLoader(dataset_Train, batch_size=24, shuffle=True, drop_last = True, num_workers=4, pin_memory = True)
    dataloader_test = torch.utils.data.DataLoader(dataset_Test, batch_size=24, shuffle=True, drop_last = True, num_workers=4, pin_memory = True)
    ...
    wandb.init(
        project = "kaggle_Auc_fixed",
        name = "Hybrid_vit_Full_2"
    )
