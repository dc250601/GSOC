...
#             self.swin_backbone.layers[i] = nn.Identity()
    ...
    def forward(self, image, stage):
        ...
        #Two new stages are added, -1 to unfreeze the swin block and freeze the embed block
        #-2 to freeze the swin block and unfreeze the embed block
        ...
        if stage == -1:
            for para in self.swin_backbone.patch_embed.parameters():
                para.requires_grad = False
...
def main():
    ...
    wandb.init(
        project = "Hybrid_Zoo",
        name = "Eff2_Swin_trained_step_by_step_rtc_taking_it_further"
    )
...
#------------------------------------------------------------------------------
    ...
    print("Loadind the pre-trained weights")
    with torch.no_grad():
        model(sample,2)
    with torch.no_grad():
        model(sample,3)
        
    checkpoint = torch.load("model_epoch_149_stage_3.pt")
    model.load_state_dict(checkpoint['model_state_dict'])    
    print("printing the pre-trained weights complete")
#------------------------------------------------------------------------------
    ...
    for epoch in range(20):
        ...
        print("----------------------------------------------------")
        PATH = f"model_epoch_{epoch}_stage_m1.pt"
        torch.save({
                'epoch': epoch,
    ...
    for epoch in range(20):
        ...
        print("The validation AUC of the epoch, %.3f"%test_auc)
        print("----------------------------------------------------")
        PATH = f"model_epoch_{epoch}_stage_m2.pt"
        torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
