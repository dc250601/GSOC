...
def main():
    ...
    wandb.init(
        project = "Ensemble",
        name = "Eff2_Swin_trained_step_by_step_rtc_run2"
    )
    ...
    #Stage 3--------------------------------------------------------------------------------
    optimizer = torch.optim.AdamW(model.parameters(), lr = 0.00005, weight_decay=0.05)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', verbose = True,threshold = 0.0001,patience = 5, factor = 0.7)
