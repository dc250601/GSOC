...
wandb.init(
    ...
    name = "Stage-2_node_2_no_shuffle"
)
...
with tpu_strategy.scope():
    model_a = Stage1(model_a, model_b, model_top)
    checkpoint = tf.train.Checkpoint(model=model_a)
    checkpoint.restore("./checkpoint/stage1-1")

    model = Stage2(model_ef, connector, model_b, model_top)

    model.compile(
    optimizer=tfa.optimizers.AdamW(learning_rate=LR_2, weight_decay=WD_2),
...
with tpu_strategy.scope():
    ...
    checkpoint.save("./checkpoint/stage2")
